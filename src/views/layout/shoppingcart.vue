...
export default {
  ...
  computed: {
    ...
    isLogin () {
      return JSON.parse(localStorage.getItem('shopping_info')).token
    }
  },
  methods: {
    ...
    changeCount (value, goodsId, skuId) {
      this.$store.dispatch('cart/changeProCount', {
        goodsId: goodsId,
        goodsSkuId: skuId,
        goodsNum: value
      })
    ...
    goPay () {
      if (this.selCount > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(item => item.id).join(',') // ‘cartId，cartId’
          }
        })
      }
    }
  ...
  created () {
    ...
    // }
    const token = JSON.parse(localStorage.getItem('shopping_info')).token
    if (token) {
      this.$store.dispatch('cart/getCartAction')
    }
  }
