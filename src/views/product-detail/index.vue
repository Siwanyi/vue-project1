  ...
  <div class="prodetail">
    ...
    <div class="comment">
      <div class="comment-title">
        <div class="left">商品评价 ({{ totalCom }}条)</div>
    ...
    <div class="footer">
      ...
      <div @click="$router.push('/shoppingcart')" class="icon-cart">
        <span v-if="cartTotal > 0" class="num">{{ cartTotal }}</span>
        <van-icon name="shopping-cart-o" />
        <span>购物车</span>
      </div>
      <div @click="addPro" class="btn-add">加入购物车</div>
      <div @click="buyPro" class="btn-buy">立刻购买</div>
    </div>
    <!-- 加入购物车弹层 -->
...
export default {
  ...
  methods: {
    ...
    async getDetail () {
      const { data: { detail, total } } = await getProDetail(this.getGoodsId)
      this.detail = detail
    ...
    async getComments () {
      const { data: { list } } = await getProComments({
